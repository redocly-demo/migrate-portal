...
async function migrate() {
  try {
    (0, import_node_child_process.execSync)("git diff --exit-code");
  } catch (e) {
...
function migrateMarkdown(fsInfo) {
  ...
  for (const filePath of markdownFiles) {
    ...
    let newContent = content.replace(admonitionRegex, (_, type2, title, text) => {
      ...
      text = text.endsWith("\n") ? text : text + "\n";
      return `{% admonition type="${type2}" name="${title}" %}
${text}{% /admonition %}`;
    });
...
${newContent.slice(len)}`;
    ...
    newContent = newContent.replace(/(\[[^\]]+\]\()([^)]+)(\))/g, (_, start, link, end) => {
      ...
      const isRefLink = Object.keys(renamedFiles).some((f) => {
...
function migrateSidebars(fsInfo) {
  ...
  for (const filePath of sidebars) {
    const content = fs.readFileSync(filePath, "utf8");
    const data = js_yaml_default.load(content);
    ...
    if (Array.isArray(data)) {
      ...
      if (!dequal(data, newData)) {
        fs.writeFileSync(filePath, js_yaml_default.dump(newData));
      }
    } else {
      Object.entries(data).forEach(([key, data2], idx) => {
        const newData = transformSidebarItems(data2, filePath);
        const prefix = idx === 0 ? "" : slug(key) + ".";
        const newName = prefix + "sidebars.yaml";
        if (!dequal(data2, newData)) {
          fs.writeFileSync(path.join(path.dirname(filePath), newName), js_yaml_default.dump(newData));
        }
      });
    }
  }
...
async function migratePackageJson() {
  ...
  let choice = -1;
  do {
    choice = parseInt(await new Promise((resolve2) => rl.question("Enter the number of the product: ", resolve2)));
...
async function migrateOpenAPI(fsInfo) {
  ...
  async function migratePageYaml(definitionId, pageYaml, targetPath, versioned) {
    ...
    if (definitionPath.match(/https?:\/\//)) {
      const dir = versioned ? path.dirname(targetPath) : path.join(path.dirname(targetPath), path.basename(targetPath, ".yaml"));
      fs.mkdirSync(dir, {
        recursive: true
      });
...
`;
  ...
  function migrateRefDocsSettings(settings) {
    return {
      ...settings,
      codeSamples: settings.generateCodeSamples,
      generateCodeSamples: void 0,
      pagination: void 0
      // FIXME
    };
  }
...
function migrateConfig() {
  ...
  if (siteConfig.apiCatalog) {
    migrationInstructions += `## API Catalog
...
`;
  ...
  const tocDepth = siteConfig.toc?.maxDepth || siteConfig.tocMaxDepth || void 0;
  const tocHide = !siteConfig.toc?.enable === false ? void 0 : true;
  const config = {
    requiresLogin: true,
    ...
    ignore,
    links: siteConfig.stylesheets ? siteConfig.stylesheets.map((l) => ({
      rel: "stylesheet",
      href: l
    })) : void 0,
    scripts: siteConfig.scripts || siteConfig.postBodyScripts ? {
      head: siteConfig.scripts ? siteConfig.scripts.map((s) => ({ src: s })) : void 0,
      body: siteConfig.postBodyScripts ? siteConfig.postBodyScripts.map((s) => ({ src: s })) : void 0
    } : void 0,
    ...
    markdown: {
      partialsFolders: partialFolders.size > 0 ? Array.from(partialFolders) : void 0,
      ...
      toc: tocDepth !== void 0 || tocHide ? {
        depth: tocDepth,
        hide: tocHide
      } : void 0
    },
    logo: siteConfig.logo ? typeof siteConfig.logo === "string" ? {
  ...
  function migrateNavbar(siteConfig2) {
    return {
  ...
  function migrateFooter(siteConfig2) {
    ...
    return {
      items: siteConfig2.footer.columns || void 0,
      copyrightText: siteConfig2.footer.copyrightText
    };
  }
}
...
`;
  ...
  function convertMdxToTsx(content, filePath) {
...
function migrateOverrides() {
  if (fs.existsSync("_overrides")) {
    migrationInstructions += `## \`_overrides\` folder
...
function slug(str2) {
  return str2.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
}
function slugRoute(str2) {
  return str2.split("/").map(slug).join("/");
}
function getFsInfo(contentDir) {
  const files = [];
...
function processFrontMatter(content, filePath) {
  ...
  if (frontmatter.redirectFrom) {
    frontmatter.redirects = frontmatter.redirectFrom.reduce((acc, str2) => {
      acc[str2] = {};
      return acc;
    }, {});
...
async function tryFetchRemoteDefinition(definitionPath) {
  ...
  if (token) {
    ...
    const body = JSON.stringify({
      query: "query GetSourceDetails($orgId: String!, $definitionName: String!, $versionName: String!) {\n    def: definitionVersionByOrganizationDefinitionAndName(\n        organizationId: $orgId,\n        definitionName: $definitionName,\n        versionName: $versionName\n    ) {\n        id\n        source\n        sourceType\n    }\n}",
      variables: {
        orgId,
        definitionName,
        versionName
...
`;
      ...
      }[details.data.def.sourceType] || "https://redocly.com/docs/realm/setup/how-to/remote-content/from-github";
      readme += `Connect remote content from [${details.data.def.sourceType} repository](${link}).    Pick \`${source.login}/${source.repo}\` repository and select \`${dirName}\` folder`;
    }
    if (readme) {
